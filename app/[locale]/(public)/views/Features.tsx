...
export default async function Features({ pageName }: { pageName: string | null }) {
  ...
  return (
    <>
      <h2 className="text-xl md:text-5xl font-bold text-left mb-4 md:mb-8 text-feature-title font-leckerli">
        {t('gameTitle')}
      </h2>
      ...
      <section className="mb-4 md:mb-8">
        ...
        <ul className="grid grid-cols-2 gap-2 md:gap-4">
          {['feature1', 'feature2', 'feature3', 'feature4'].map((feature, index) => (
            <li
              key={index}
              className="bg-feature-card hover:bg-feature-card-hover transition-colors p-2 md:p-4 rounded-lg"
            >
              <h3 className="text-base md:text-xl font-semibold mb-1 md:mb-2 text-feature-title">
                {t(`${feature}Title`)}
              </h3>

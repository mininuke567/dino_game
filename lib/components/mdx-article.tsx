...
function Table({ data }: { data: any }) {
  const headers = data.headers.map((header: any, index: number) => (
    <th key={index} className="text-table-header bg-table-header-bg border-table-border">
      {header}
    </th>
  ));
  const rows = data.rows.map((row: any, index: number) => (
    <tr key={index} className="hover:bg-table-row-hover">
      {row.map((cell: any, cellIndex: number) => (
        <td key={cellIndex} className="text-table-cell border-table-border">
          {cell}
        </td>
      ))}
    </tr>
...
function RoundedImage(props: any) {
  return <Image alt={props.alt} className="rounded-lg border border-image-border" {...props} />;
}
...
function Code({ children, ...props }: { children: any }) {
  ...
  return (
    <code
      className="bg-code-bg text-code font-mono p-1 rounded"

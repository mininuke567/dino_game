      ...
      <div className="bg-background text-foreground min-h-screen pt-5 pb-5">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-4 mb-6">
            {category.icon && <Image src={category.icon} alt={category.name} className="w-8 h-8" />}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {currentGames.map((game) => (
              <GameCard key={game.url} game={game} isHorizontal={isHorizontal} />
            ))}
          </div>
              ...
              {paginationLinks.map((link) => (
                <Link
                  ...
                  className={`px-4 py-2 rounded-lg transition-colors ${
                    link.isCurrent ? 'bg-primary text-primary-foreground' : 'bg-default-100 hover:bg-default-200'
                  }`}
                >
                  {link.page}
                </Link>

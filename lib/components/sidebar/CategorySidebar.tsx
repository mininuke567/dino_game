    ...
    >
      <ScrollShadow className="h-full overflow-hidden">
        <div className="flex flex-col p-4 space-y-2">
          ...
          >
            <IconChevronLeft className={cn('w-4 h-4 transition-transform duration-300', !isExpanded && 'rotate-180')} />
          </button>
          ...
          {categories.map((category) => (
            ...
            >
              <span className="shrink-0">
                {category.icon ? (
                  <Image src={category.icon} alt={category.name} className="w-6 h-6" />
                ) : (
